...
public class InstructorCreateAssignmentsController implements UserAwareController {
    ...
    private void handleBack() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/learningmanagementsystem/instructor-home.fxml"));
            Parent root = loader.load();

            Object controller = loader.getController();
            if (controller instanceof UserAwareController) {
                ((UserAwareController) controller).initUserData(instructor);
            }
            ...
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Failed to load instructor home screen.");
        }
    }
