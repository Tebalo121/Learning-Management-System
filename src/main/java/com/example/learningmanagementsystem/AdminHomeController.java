...
public class AdminHomeController implements UserAwareController {
    ...
    private void handleAnnouncements() {
        log("Navigating to Announcements");
        switchScene("admin_announcements.fxml");
    }

    @FXML
    private void handleBackup() {
        log("Initiating Database Backup");
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Backup");
        alert.setHeaderText("Backup Complete");
        alert.setContentText("Database backup has been created successfully.");
        alert.showAndWait();
    }

    @FXML
    private void handleFeedback() {
        log("Navigating to Feedback Management");
        switchScene("admin_feedback.fxml");
    }
